...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    sentences = source_text.split('\n')
    source_ids = [[source_vocab_to_int[word] for word in sentence.split()] for sentence in sentences]
    print(len(source_ids))
    target_sens = target_text.split('\n')
    for index,sentence in enumerate(target_sens):
        target_ids.append([target_vocab_to_int[word] for word in sentence.split()])
        target_ids[index].extend([target_vocab_to_int['<EOS>']])
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    def cell():
        ...
        drop_out = tf.contrib.rnn.DropoutWrapper(cell,keep_prob)
        return drop_out
    encoder = tf.contrib.rnn.MultiRNNCell([cell() for _ in range(num_layers)])
    encoder_output,encoder_state = tf.nn.dynamic_rnn(encoder,embed_input,sequence_length=source_sequence_length,dtype=tf.float32)
...
def decoding_layer(dec_input, encoder_state,
    ...
    embed_input = tf.nn.embedding_lookup(dec_embeddings, dec_input)

    # 2. Construct the decoder cell
    def cell():
        cell = tf.contrib.rnn.LSTMCell(rnn_size,initializer=tf.random_uniform_initializer(-0.1, 0.1))
        drop_out = tf.contrib.rnn.DropoutWrapper(cell,output_keep_prob=keep_prob)
        return drop_out
    decoder_cell = tf.contrib.rnn.MultiRNNCell([cell() for _ in range(num_layers)])
    ...
    with tf.variable_scope("decoder"):
        training_decoder =  decoding_layer_train(encoder_state, 
                                                 ...
                                                 output_layer, 
                                                 keep_prob)
    #print(training_decoder.get_shape().as_list())
    with tf.variable_scope("decoder",reuse=True):
        ...
        infer_decoder = decoding_layer_infer(encoder_state, 
                                             ...
                                             keep_prob)
    # TODO: Implement Function
    #print(infer_decoder.get_shape().as_list())
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size,
    ...
    decoder_intput = process_decoder_input(target_data, target_vocab_to_int, batch_size)
    
    training_decoder_output,inference_decoder_output = decoding_layer(decoder_intput, encoder_state, 
                                                                      target_sequence_length, 
                                                                      max_target_sentence_length,
                                                                      rnn_size, num_layers,
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    word_to_ids = [vocab_to_int.get(word,vocab_to_int['<UNK>']) for word in sentence.split()]
    return word_to_ids
